...
const Inbox = () => {
	...
	const { setStagedFiles, setConfirmedFiles } = useContext(confirmationContext);
	const PAGE_SIZE = 3;
	const [requests, setRequests] = useState({});
	const [isLoading, setLoading] = useState(false);
	const { account } = useWeb3React();

	const handleConfirm = (requestID, addresses, selectedFiles, setSelected) => {
			...
			.send({ from: account }, (error, result) => {
				if (!error) {
					setStagedFiles({
						...
						txhash: result,
						requestID,
						contributions: selectedFiles,
					});
					setSelected((values) => {
						let result = {};
						Object.keys(values).forEach((key) => (result[key] = []));
						return result;
					});
		...
		appState.contract.events
			.Transfer({})
			.on('data', (event) => {
				setConfirmedFiles(event.transactionHash);
