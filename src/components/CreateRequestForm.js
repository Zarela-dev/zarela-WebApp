...
const CreateRequestForm = React.forwardRef(
	({ children, formik, appState }, ref) => {
		const [selectedOption, setSelectedOption] = useState([]);
		const formikRef = useRef();

		useEffect(() => {
			if (appState.oldDataForm !== undefined) {
				const data = JSON.parse(appState.oldDataForm);
				console.log('inner context ', data.title);
				formik.setFieldValue('title', data.title);
				formik.setFieldValue('desc', data.desc);
				formik.setFieldValue('angelTokenPay', data.angelTokenPay);
				formik.setFieldValue('laboratoryTokenPay', data.laboratoryTokenPay);
				formik.setFieldValue('instanceCount', data.instanceCount);
				formik.setFieldValue(
					'category',
					data.category.map((item) => item)
				);
				setSelectedOption(data.category.map((item) => item));
			}
